...
- `crates/outline-transport/`: WebSocket, HTTP/2, HTTP/3, raw QUIC, VLESS,
  XHTTP, direct Shadowsocks transport logic, resume caches и общий dial code.
...
- `vendor/sockudo-ws`: vendored path dependency. Относитесь к vendor-коду как к
  внешнему upstream-коду: не меняйте его без явной причины и следите, чтобы
  workspace checks случайно не превращали vendor в основную область рефакторинга.
...
- `crates/outline-transport/src/lib.rs` является публичным facade и сейчас
  содержит заметную часть dial implementation. Новые transport branches,
  downgrade planning и длинные списки аргументов лучше выносить в typed options
  и отдельные planning/execution modules.
