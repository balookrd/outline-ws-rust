...
- Cross-transport session resumption — client side, transport-layer plumbing. WebSocket Upgrade requests over HTTP/1.1, HTTP/2 and HTTP/3 now carry `X-Outline-Resume-Capable: 1` so that an outline-ss-rust server with the matching feature enabled can mint a Session ID and return it via `X-Outline-Session`. The ID is surfaced on `WsTransportStream::issued_session_id()` for callers to stash. A new `connect_websocket_with_resume` accepts an `Option<SessionId>` and threads it as `X-Outline-Resume: <hex>` on the upgrade so the server can re-attach to a parked upstream — opt-in, leaving the existing `connect_websocket_with_source` as a no-resume wrapper. Higher-level uplink integration (per-uplink Session ID cache, automatic resume on reconnect) is not yet wired; this revision lands the foundation. See `docs/SESSION-RESUMPTION.md` in outline-ss-rust for the wire format.
