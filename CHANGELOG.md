...
- Cross-transport fallback in the dial / chunk-0 failover loop. The candidate list returned by `tcp_candidates` / `udp_candidates` / `tcp_failover_candidates` is now stably grouped by `UplinkTransport` (`vless` / `shadowsocks` / `ws`) in order of first appearance, while the relative order inside each group still reflects the underlying health/weight/score ranking. Consumers (`serve_tcp_connect`, `select_udp_transport`, the chunk-0 `failover_to_next_candidate` step) iterate the list with a shared `tried_indexes` set, so they now exhaust every endpoint of the leading transport before falling over to the next one — e.g. all VLESS uplinks are tried before any Shadowsocks/WS endpoint, and the cross-transport fallback only kicks in when a second transport is actually configured in the group. Previously a chunk-0 stall on one VLESS endpoint could jump straight to a Shadowsocks endpoint while a second healthy VLESS endpoint sat untried, because the interleaved health/weight ordering pulled it lower in the list. Strict `active_passive` mode and probe-driven health are unchanged: the new helper only re-orders an already-ranked list at the dial-loop level, the leader of the list (and therefore the "current" transport) is still picked by health/weight/score. Two regressions pin the grouping for both transports: `tcp_candidates_are_grouped_by_transport_kind`, `udp_candidates_are_grouped_by_transport_kind`.
- Documentation reference for `[outline]` configuration shape and load balancing. New sections in `docs/UPLINK-CONFIGURATIONS.md` (and the Russian mirror): "Top-level `[outline]` shape" — describes the inline single-uplink shorthand vs the multi-uplink + groups production shape, and the `outline.transport` enum (`ws` / `shadowsocks` / `vless`); "Load-balancing reference" — full field-by-field table for `[outline.load_balancing]` (legacy single-group surface) and the equivalent fields under `[[uplink_group]]` (multi-group surface), with defaults sourced from `src/config/load/balancing.rs` and `crates/outline-transport/src/vless/udp_mux.rs`, plus a routing-scope cheat sheet (`per_flow` / `per_uplink` / `global`) and the `mode` × `scope` interaction matrix. `config.toml` now ships a commented-out `[outline.load_balancing]` template (legacy shape) and an inline-`[outline]` shorthand example next to the `[[outline.uplinks]]` array; `config-router.toml` cross-refs the new doc section. Surfaces previously had to be discovered by reading `src/config/schema.rs:348` and `src/config/load/balancing.rs:9` directly.
- `[outline.probe.http]` accepts a `urls = [...]` rotation list in addition to the single `url = "..."` field. The probe advances through the list one entry per call (atomic cursor on `HttpProbeConfig`, shared across uplinks in the group), so consecutive probe calls hit consecutive endpoints. Spreading load across multiple targets surfaces per-site outages instead of masking them behind one always-reachable URL, and warm-keepalive ticks rotate through the same list. Either `url` or `urls` may be set; `urls` wins if both are present. Status remains 2xx/3xx → success (any HTTP status in `200..400`).
- Per-uplink override for the browser fingerprint diversification strategy. Each `[[outline.uplinks]]` entry accepts an optional `fingerprint_profile` key with the same string aliases as the top-level knob (`"off"`, `"stable"`, `"random"`, …); omitting it inherits the top-level value. Useful when one uplink must keep a byte-identical xray-style wire shape while siblings on the same `host:port` opt into per-host-stable identities — siblings can no longer flip each other's profile via the global. Plumbing rides on a new `tokio::task_local!` scope inside `outline_transport::fingerprint_profile`: `with_strategy_override(strategy, fut)` runs `fut` with `select(url)` reading `strategy` instead of the process-wide value, and the override naturally drops on `.await` completion (it does not leak into spawned post-handshake tasks like drivers or body-drain loops, which is exactly what we want — `select` only fires at the dial entry-point). The new `outline_uplink::dial::dial_in_uplink_scope(uplink, fut)` helper wraps every transport dial site that has an `UplinkConfig` in scope: WS dial-and-resume, WS warm-standby refill, VLESS / SS over QUIC, SS UDP over QUIC, and the matching probe variants. Tests pin: task-local override supersedes the default global, `Strategy::None` override disables selection inside the scope, nested scopes pick the innermost value, `load_config` threads explicit `"stable"` / `"off"` per-uplink overrides into the resolved `UplinkConfig` while leaving inheriting uplinks at `None`. Documented in `docs/UPLINK-CONFIGURATIONS.md` "Per-uplink override".
- Per-host browser fingerprint diversification for the WS / XHTTP dial paths. WS H1 / H2 / H3 upgrades and XHTTP H1 / H2 / H3 GET / POST requests can now mix in browser-style identification headers (`User-Agent`, `Accept`, `Accept-Language`, `Accept-Encoding`, the Sec-CH-UA family, and the matching `Sec-Fetch-{Site,Mode,Dest}` triplet — `mode=websocket,dest=websocket` for WS upgrades, `mode=cors,dest=empty` for XHTTP), so passive DPI rules keying on "WS upgrade missing User-Agent" or "XHTTP POST missing browser headers" stop separating this client from real browser traffic. The pool ships six representative profiles — Chrome 130 (Windows + macOS), Firefox 130 (Windows + macOS), Safari 17 (macOS), Edge 130 (Windows) — chosen so a per-host-stable selector lands on a Chromium identity for ~⅔ of peers and a Gecko / WebKit identity for the remaining ⅓, matching rough real-world browser-share. Selection is keyed by `(host, port)` via `DefaultHasher` and sticks for the lifetime of the process, so a single peer never sees one session split across two fingerprints. The knob is opt-in via the new top-level `fingerprint_profile` config key — accepts `"off"` / `"none"` / `"disabled"` (default — wire shape byte-identical to pre-knob builds), `"stable"` / `"per_host_stable"` / `"per-host-stable"` / `"per-host"` (one identity per host:port), or `"random"` (fresh profile per dial). Strings round-trip through `serde::Deserialize` so the value comes straight from `config.toml`. The bootstrap binary calls `init_fingerprint_profile_strategy` once at startup, alongside the existing `init_downgrade_ttl` wiring; embedded callers (tests, custom binaries) can wire the strategy directly via the Rust API. Cross-transport session-resumption headers (`X-Outline-Resume-*`) are inserted independently of the profile and ride on top of whatever identity is active. Deliberately NOT covered (separate, costlier work): TLS ClientHello / JA3 / JA4 — rustls does not expose cipher / extension / curve order, so meaningful TLS diversification requires a uTLS-style stack (`boring` / BoringSSL); ALPN ordering — currently fixed per carrier, the TLS configs are cached per ALPN list; HTTP/2 SETTINGS frame fingerprint (Akamai / JA4H2) — owned by the `h2` crate and largely closed to client-side tweaks; QUIC transport-parameter ordering — owned by `quinn`. Documented in detail in `docs/UPLINK-CONFIGURATIONS.md` "Browser fingerprint diversification".
- Inline `stream-one → packet-up` fallback for the XHTTP carrier, plus a per-host submode cache (`xhttp_submode_cache`). When a `?mode=stream-one` dial fails on `xhttp_h2` / `xhttp_h3` after the carrier handshake succeeds, the dialer retries packet-up on the **same** TCP/TLS/h2 (or QUIC/h3) connection — no fresh handshake, just a different request shape — and records the failure in a per-host cache keyed by destination `(host, port)`. Subsequent dials to the same host skip stream-one upfront for `mode_downgrade_secs` and go straight to packet-up, avoiding the doomed handshake on every connect. The cache is the orthogonal sibling of `xhttp_mode_cache`: independent slot, independent TTL, independent decay, so a stream-one block does not lower the h-version cap and an h-version downgrade does not refresh the stream-one block. Cleared early by a successful stream-one dial. The `xhttp_h1` carrier silently coerces stream-one to packet-up at the public `connect_xhttp` entry point (h1 cannot multiplex a streaming GET against a streaming POST on a single connection); the inner h1 driver retains its defensive `packet-up only` bail for direct callers. Effective submode is published on the snapshot (`tcp_xhttp_submode` / `udp_xhttp_submode` configured + `tcp_xhttp_submode_block_remaining_ms` / `udp_xhttp_submode_block_remaining_ms`) and rendered on the dashboard's protocol pill — `stream-one` shows as `/S`, packet-up has no suffix, and a live block renders as `/S↘P` so the silent downgrade is visible. Motivating use case: middleboxes (CDNs, corporate proxies, mobile NAT idle-timeouts) that buffer or close streaming POST bodies but accept short POSTs unchanged — stream-one stalls there while packet-up survives, and the cache turns a per-dial guess into a per-host learned shape.
- Per-host XHTTP downgrade cache (`xhttp_mode_cache`) — sibling of the existing WS-only `ws_mode_cache`. Records `xhttp_h3` / `xhttp_h2` failures keyed by destination `(host, port)` so subsequent dials of the same upstream from different uplinks (e.g. several VLESS UUIDs behind one CDN host) skip the doomed handshake without each waiting for its own per-uplink window to fill. Each chain has its own slot — a `WsH3` failure no longer counts against an XHTTP cap and vice versa, so multiple transports against the same `host:port` can decay independently. Shares the `mode_downgrade_secs` knob with the WS cache (one knob, two slots) and is GC'd from the same `gc_shared_connections` cadence. Cleared early by `record_success` on a meets-or-exceeds dial — a recovered `xhttp_h3` reachability immediately drops the cap so the next dial actually exercises h3 again instead of waiting for TTL expiry.
- Per-uplink mode-downgrade window now covers the XHTTP family. The window guard previously only opened on `WsH3` / `Quic` failures; it now also opens on `XhttpH3` and `XhttpH2` failures, so subsequent dials skip the doomed handshake until the TTL expires (default 60 s, governed by `mode_downgrade_secs`). Implementation switches `effective_tcp_mode` / `effective_udp_mode` from a hardcoded `WsH2` return to a family-aware ceiling tracked in the new `PerTransportStatus::mode_downgrade_capped_to` field — `WsH3` / `Quic` collapse to `WsH2`, `XhttpH3` collapses to `XhttpH2`, `XhttpH2` to `XhttpH1`. Multi-step XHTTP downgrades (`XhttpH3 → XhttpH2 → XhttpH1`) converge over consecutive dials: each silent-fallback observation lowers the cap one rank inside the active window and never raises it. The cap is published through `UplinkSnapshot::tcp_mode_capped_to` / `udp_mode_capped_to`, so the dashboard's `tcp_mode_effective` / `udp_mode_effective` columns now display the actual carrier the dispatcher will use (instead of the hardcoded `ws_h2` / `xhttp_h2` decoration). Defensive: triggers that would *raise* the configured mode (e.g. a stray `WsH3` fallback note on a `WsH2`-configured uplink) or cross families (an XHTTP failure note on a WS uplink) are skipped — they indicate a wiring bug upstream and must not mis-park the uplink.
- VLESS-over-XHTTP `xhttp_h1` packet-up carrier and the `xhttp_h3 → xhttp_h2 → xhttp_h1` fallback chain. New `vless_mode = "xhttp_h1"` selects HTTP/1.1 packet-up directly; the existing `xhttp_h2` and `xhttp_h3` arms in `connect_websocket_with_resume` now fall through to it when the h2 dial fails (in addition to the existing `xhttp_h3 → xhttp_h2` step). The h1 carrier is the last-resort fallback for paths blocking both QUIC and h2 ALPN — wire URL stays identical (`<base>/<session>/<seq>`) so the same `xhttp_path_vless` listener serves it. Because HTTP/1.1 cannot multiplex a streaming GET against concurrent POSTs on a single connection, the driver dials **two** keep-alive sockets per session: one dedicated to the long-lived downlink GET (chunked response), and one to strictly serialised uplink POSTs (no pipelining — too brittle through CDN/proxy intermediaries). Throughput is bounded by single-stream POST round-trip time and is expected to lag h2 noticeably under load. The same `X-Outline-Resume` token threads through every carrier switch, so a parked VLESS upstream re-attaches across the full chain. Stream-one is intentionally not implemented for h1 — `?mode=stream-one` paired with `vless_mode = xhttp_h1` (or a chain that falls through to h1) bails at dial time. VLESS share-link URIs accept `alpn=h1` / `alpn=http/1.1` to pin the h1 carrier directly. CLI / TOML / control-plane payloads accept `xhttp_h1` everywhere `xhttp_h2` / `xhttp_h3` are accepted.
...
- `load_balancing.global_udp_strict_health` (default `false`) — controls whether UDP probe failures and UDP cooldown gate the active uplink alongside TCP in `routing_scope = "global"`. The new lenient default treats UDP health as informational only: a TCP-healthy active uplink keeps its slot even when its UDP probe is flapping (UDP traffic continues to route through it and surfaces failures naturally at the data plane); strict gating remains available for operators that want the legacy behaviour. The change matches the existing documented intent of global scope ("Global routing should primarily follow TCP quality. UDP only acts as a weak tie-breaker" — comment in `global_selection_score_latency`) and closes a cascade-flap mode where flaky UDP paths (most commonly VLESS-XHTTP/H3 over a network that drops or rate-limits QUIC, broken UDP NAT, mobile carriers with deep middlebox interference) repeatedly demoted the active uplink: each newly-promoted backup got the same UDP probe failure on the same network path within seconds and was demoted in turn, producing the dense Active-Uplink-Timeline toggling visible on the dashboard with role-badge `global: failover: UDP probe marked active unhealthy`. Three regression tests pin the new behaviour: `global_scope_keeps_active_when_udp_unhealthy_by_default` covers the default lenient path, while the renamed `global_scope_with_strict_udp_*` trio (cooldown / probe-unhealthy / udp-down-vs-tcp-selected) confirms that opting back in via `global_udp_strict_health = true` restores the pre-1.4.x semantics. Cross-test infrastructure updated everywhere `LoadBalancingConfig` is built by hand to default the new field to `false`. The knob is honoured at both the top-level `[load_balancing]` block and per-`[[uplink_group]]`. No effect when `routing_scope` is `per_flow` or `per_uplink` — those scopes already gate per-transport health independently.
- `load_balancing.runtime_failure_window_secs` (default 60) — time window over which `consecutive_runtime_failures` are counted toward the strict-global health-flip escalation. A new runtime failure arriving more than this window after the previous one resets the streak to 1 instead of incrementing. Set to `0` to keep the legacy behaviour (counter only resets on real data transfer or a successful probe). Closes a flapping mode on low-traffic strict-global uplinks where two unrelated transient errors spaced minutes apart would stack into a `>= probe.min_failures` health flip on the active uplink and start a cascade of failovers through the whole pool: each new active picked up its own pair of sparse errors and got demoted in turn, producing the dense Active-Uplink-Timeline toggling visible on the dashboard. The runtime-failure threshold itself is still bound to `probe.min_failures` (one knob, two signals — operator expectation: "after N failed attempts the uplink is considered down"), but the streak is now bounded in *time* as well as in *count*. Implementation rides on a new `PerTransportStatus::last_runtime_failure_at` timestamp and a one-line decay check before the increment in `report_runtime_failure`. New regression `sparse_runtime_failures_do_not_escalate_to_health_flip` pins the new behaviour; counter-regression `sparse_runtime_failures_still_escalate_when_window_is_zero` pins that `runtime_failure_window_secs = 0` keeps the legacy stacking. Default of 60 s is one probe interval at the typical `probe.interval_secs = 30` plus headroom — sized so a probe cycle reliably resets the streak through the success path, while real bursts of back-to-back errors (which arrive faster than the window) still escalate immediately.

### Fixed

- Global routing scope flapped between uplinks under steady noise because `global_selection_score_latency` deliberately ignored the decayed failure penalty for *every* scenario. This is correct for `auto_failback = true` — under load the active uplink's EWMA inflates while the idle backup retains a low probe-derived EWMA, so a residual penalty would make the active look permanently worse and starve weight-driven failback (see `candidates.rs` failback path). It is wrong for `auto_failback = false` (the default), where the active is sticky as long as it is healthy and the score is only consulted to pick a backup on failover or initial selection: ignoring the penalty there means failover can land on a backup that itself just failed, which then fails again, producing the rapid Active-Uplink-Timeline toggling visible on the dashboard. Fixed by gating penalty inclusion on `auto_failback`: with `auto_failback = false` the global selection score now uses the full `score_latency` (base EWMA + decayed `failure_penalty`); with `auto_failback = true` it stays on raw EWMA. Test `global_scope_ignores_penalty_in_selection_score` was split into two — the auto-failback case keeps the old "primary wins despite penalty" assertion, the new without-auto-failback case asserts the opposite. No config knob change; existing `failure_penalty_ms` / `failure_penalty_halflife_secs` now actually shape global-scope selection in the default mode.
- XHTTP uplink (h1 / h2 / h3) hung mid-transfer after roughly 32 messages on bulk uploads. The `Sink` impl on `XhttpStream` reported `Poll::Ready` from `poll_ready` unconditionally and used `try_send` in `start_send`, returning `Err("xhttp outgoing buffer full")` once the per-session outbound channel filled up. The TCP / WS writer task above us treats any Sink error as fatal and exits, leaving the data channel filling up against a dead consumer — visible as a transfer that stalls a few chunks in and never recovers. Fixed by wrapping the outbound `mpsc::Sender` in `tokio_util::sync::PollSender`, which reserves a permit asynchronously and stashes the waker, so bulk uploads now apply real back-pressure all the way down to the caller. Same commit raises the in-memory burst windows for the XHTTP and WS data paths from `32 / 8 / 64` to `256` (inbound, outbound, stream-one request body, WS writer data channel) — sized for ~4 MB inflight at the 16 KB SS2022 chunk boundary so a 1 Gbps × 30 ms link doesn't hand the lock back after one chunk; h2 / h3 / native-WS flow control on the wire remains the real upper bound. Both writer tasks (`tcp_transport::writer::transport::WsWriteTransport::spawn` and `frame_io_ws::spawn_ws_writer`) now log the wire-side error before exiting instead of disappearing silently. Regression coverage in `xhttp/tests/sink_backpressure.rs`: a full outbound channel must pend instead of erroring, a dropped receiver still surfaces as a Sink error.
