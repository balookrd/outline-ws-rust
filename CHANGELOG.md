...
- **`bypass_when_down` — group-level tunnel bypass while every uplink in the group is down.** New boolean on `[[uplink_group]]` (and on legacy `[outline.load_balancing]`), default `false`. While the group has no healthy uplink, traffic whose route resolves to the group is dispatched `direct` — out the host's own networking stack with `direct_fwmark` applied, exactly like a `via = "direct"` route — instead of failing on a dead group; the decision is re-evaluated live per TCP dial / UDP datagram / TUN flow, so traffic returns to the tunnel as soon as any uplink recovers. The health criterion matches the existing route-fallback decision: per-transport on the SOCKS5 path, both-transports-down on the TUN path (same as `tun_suppress_icmp_reply_when_down`). An explicit `fallback_via` / `fallback_direct` / `fallback_drop` on the matching `[[route]]` rule takes precedence; the bypass then still applies one level deep to the group the fallback lands on. Works without `[[route]]` too — the implicit default-group dispatch honours the flag, and the SOCKS5 UDP per-association direct socket is now pre-allocated whenever any group opted in. With `tun_suppress_icmp_reply_when_down` set alongside, pings keep being answered while the bypass is active (the path is alive, just not tunnelled). On Linux+TUN a startup warning fires when the flag is set without `direct_fwmark`, mirroring `tun.ipsec_bypass`.
