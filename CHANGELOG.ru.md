...
- Справочная документация по структуре секции `[outline]` и балансировке нагрузки. Новые разделы в `docs/UPLINK-CONFIGURATIONS.ru.md` (и в английском зеркале): «Структура секции `[outline]`» — описывает inline-стенограмму на один аплинк vs продакшен-форму с `[[outline.uplinks]]` + `[[uplink_group]]`, плюс enum `outline.transport` (`ws` / `shadowsocks` / `vless`); «Справочник балансировки нагрузки» — полная таблица полей по `[outline.load_balancing]` (legacy single-group поверхность) и эквивалентных полей под `[[uplink_group]]` (multi-group поверхность), с дефолтами, взятыми из `src/config/load/balancing.rs` и `crates/outline-transport/src/vless/udp_mux.rs`, плюс шпаргалка по `routing_scope` (`per_flow` / `per_uplink` / `global`) и матрица взаимодействия `mode` × `scope`. В `config.toml` теперь есть закомментированный шаблон `[outline.load_balancing]` (legacy-форма) и пример inline-`[outline]`-формы рядом с массивом `[[outline.uplinks]]`; `config-router.toml` ссылается на новый раздел документации. Раньше эти поверхности приходилось искать прямо в `src/config/schema.rs:348` и `src/config/load/balancing.rs:9`.
- `[outline.probe.http]` принимает список `urls = [...]` для ротации в дополнение к одиночному `url = "..."`. Проба идёт по списку по одному URL за вызов (атомарный курсор на `HttpProbeConfig`, общий на все аплинки группы), поэтому подряд идущие probe-вызовы попадают в подряд идущие эндпоинты. Размазывание нагрузки по нескольким целям помогает заметить отказ конкретного сайта вместо того, чтобы маскировать его за единственной всегда-доступной точкой; warm-keepalive тики используют ту же ротацию. Можно задать любой из вариантов; если указаны оба, выигрывает `urls`. Критерий успеха прежний: HTTP-статус в диапазоне `200..400`.
- `load_balancing.global_udp_strict_health` (по умолчанию `false`) — управляет тем, гейтит ли UDP-здоровье (probe failures + UDP cooldown) активный аплинк вместе с TCP в `routing_scope = "global"`. Новый дефолт — мягкий: UDP-здоровье информативное, при TCP-здоровом active аплинк не выкидывается, даже если его UDP-проба флипает (UDP-трафик продолжает идти через active и фейлится «честно» уже на data-plane); строгий гейт можно вернуть включением `true`. Изменение соответствует уже задокументированному смыслу global-скоупа («Global routing should primarily follow TCP quality. UDP only acts as a weak tie-breaker» — комментарий в `global_selection_score_latency`) и закрывает каскадный флап на нестабильных UDP-путях (чаще всего VLESS-XHTTP/H3 поверх сетей, которые режут или rate-limit'ят QUIC, кривые UDP-NAT'ы, мобильные операторы с агрессивными middlebox'ами): каждый поднятый бэкап получал тот же UDP probe failure на том же сетевом пути и сам через секунды демотился, отсюда и дробный toggling на Active Uplink Timeline на дашборде с role-badge `global: failover: UDP probe marked active unhealthy`. Три регрессии пинят новое поведение: `global_scope_keeps_active_when_udp_unhealthy_by_default` покрывает дефолтный мягкий путь, переименованная тройка `global_scope_with_strict_udp_*` (cooldown / probe-unhealthy / udp-down-vs-tcp-selected) подтверждает, что включение `global_udp_strict_health = true` возвращает pre-1.4.x семантику. Везде, где `LoadBalancingConfig` собирается руками в тестах, добавлено `global_udp_strict_health: false`. Knob уважается и в top-level `[load_balancing]`, и per-`[[uplink_group]]`. На `per_flow` / `per_uplink` не влияет — там здоровье per-transport и так гейтится независимо.
- `load_balancing.runtime_failure_window_secs` (по умолчанию 60) — окно, в пределах которого считаются `consecutive_runtime_failures` для эскалации в health-flip в strict-global. Новая runtime-ошибка, пришедшая позже этого окна после предыдущей, сбрасывает стрик до 1 вместо инкремента. `0` — отключает затухание (старая семантика, счётчик обнуляется только на реальной передаче данных или на успешной пробе). Закрывает флаппинг-режим на малотрафиковых strict-global аплинках: две не связанные между собой транзиентные ошибки, разнесённые на минуты, складывались в `>= probe.min_failures` и форсили health flip активного аплинка, после чего каскадом валились все остальные (новый активный быстро набирал свою пару редких ошибок и тоже демотился) — это и был частый toggling на Active Uplink Timeline на дашборде. Сам порог runtime-failure по-прежнему привязан к `probe.min_failures` (один knob, два сигнала — оператор ожидает «после N неудачных попыток аплинк считается упавшим»), но стрик теперь ограничен **и по времени**, не только по количеству. Реализация — новое поле `PerTransportStatus::last_runtime_failure_at` плюс одна проверка decay перед инкрементом в `report_runtime_failure`. Регрессия `sparse_runtime_failures_do_not_escalate_to_health_flip` пинит новое поведение; counter-регрессия `sparse_runtime_failures_still_escalate_when_window_is_zero` пинит, что `runtime_failure_window_secs = 0` сохраняет старую семантику. Дефолт 60 c — это один интервал пробы (`probe.interval_secs = 30`) с запасом: по идее success-путь пробы успевает сбросить стрик в каждом цикле, а реальные пачки back-to-back ошибок (которые приходят быстрее окна) всё ещё эскалируют сразу.
- Per-uplink override стратегии диверсификации браузерного фингерпринта. Каждый блок `[[outline.uplinks]]` принимает опциональный ключ `fingerprint_profile` с теми же строковыми алиасами, что и top-level knob (`"off"`, `"stable"`, `"random"`, …); если ключ опущен — наследуется top-level значение. Полезно, когда один аплинк должен оставаться байт-в-байт совместимым с xray-формой, а соседи на тот же `host:port` хотят PerHostStable — соседи больше не могут флипать профиль друг другу через глобал. Проводка едет через новый `tokio::task_local!` scope внутри `outline_transport::fingerprint_profile`: `with_strategy_override(strategy, fut)` гоняет `fut` так, что `select(url)` читает `strategy` вместо process-wide значения, и override естественно снимается на завершении `.await` (он не утекает в spawned post-handshake таски — драйверы, body-drain loops, — что именно то, что нужно: `select` срабатывает только в dial-entry-point). Новый helper `outline_uplink::dial::dial_in_uplink_scope(uplink, fut)` оборачивает каждый transport dial site, у которого `UplinkConfig` в области видимости: WS dial-and-resume, WS warm-standby refill, VLESS / SS поверх QUIC, SS UDP поверх QUIC, и соответствующие probe-варианты. Тесты пинят: task-local override побеждает дефолтный глобал, `Strategy::None` override отключает выбор внутри scope, вложенные scope'ы используют innermost-значение, `load_config` прокидывает явные `"stable"` / `"off"` per-uplink override'ы в собранный `UplinkConfig`, а наследующие аплинки остаются с `None`. Описано в `docs/UPLINK-CONFIGURATIONS.ru.md` «Per-uplink override».
...
- В `routing_scope = "global"` активный аплинк дёргался при штатном шуме RTT, потому что `global_selection_score_latency` намеренно игнорировал затухающий `failure_penalty` для **всех** случаев. Для `auto_failback = true` это правильно: под нагрузкой EWMA активного раздувается, а у idle-бэкапа остаётся низкий probe-derived EWMA, и любой остаточный penalty делает active вечно «хуже» — weight-based failback не сработает (см. ветку failback в `candidates.rs`). Для `auto_failback = false` (дефолт) это вредно: active и так sticky, пока он здоров, и selection score нужен только чтобы выбрать бэкап на failover или при первичном выборе. Игнорирование penalty там приводит к тому, что failover приземляется на бэкап, который сам только что фейлился, и через секунду фейлится снова — отсюда и постоянное переключение на Active Uplink Timeline. Починено гейтом по `auto_failback`: при `auto_failback = false` global selection score теперь использует полный `score_latency` (base EWMA + затухающий `failure_penalty`); при `auto_failback = true` остаётся raw EWMA. Тест `global_scope_ignores_penalty_in_selection_score` разделён на два — auto-failback кейс сохраняет старую проверку «primary побеждает несмотря на penalty», новый кейс без auto-failback проверяет противоположное. Конфиг не меняется; существующие `failure_penalty_ms` / `failure_penalty_halflife_secs` теперь реально влияют на выбор в global-скоупе в дефолтном режиме.
- XHTTP-аплинк (h1 / h2 / h3) зависал посреди передачи примерно после 32 сообщений на bulk-аплоадах. Реализация `Sink` для `XhttpStream` всегда возвращала `Poll::Ready` из `poll_ready` и использовала `try_send` в `start_send` — как только per-session outbound-канал заполнялся, возвращался `Err("xhttp outgoing buffer full")`. Writer-task выше (TCP / WS) трактует любую ошибку Sink как фатальную и завершается, после чего data-канал тихо забивается у мёртвого consumer'а — снаружи это выглядит как передача, застрявшая через несколько чанков без восстановления. Починено заворачиванием outbound `mpsc::Sender` в `tokio_util::sync::PollSender`: он резервирует permit асинхронно и стэшит waker, поэтому bulk-аплоады теперь получают настоящий back-pressure всю дорогу до вызывающего. В том же коммите подняты in-memory burst-окна XHTTP и WS data-путей с `32 / 8 / 64` до `256` (inbound, outbound, тело stream-one POST'а, data-канал WS-writer'а) — размер рассчитан на ~4 MB inflight при 16 KB SS2022-чанке, чтобы 1 Gbps × 30 ms линк не отдавал управление после одного чанка; реальный потолок по-прежнему ставит h2 / h3 / native-WS flow control. Оба writer-task'а (`tcp_transport::writer::transport::WsWriteTransport::spawn` и `frame_io_ws::spawn_ws_writer`) теперь логируют wire-ошибку перед выходом вместо тихого `return`. Регрессии закрыты в `xhttp/tests/sink_backpressure.rs`: переполненный outbound-канал должен pending'иться, а не ошибиться, а закрытый receiver — всё ещё Sink-ошибка.
