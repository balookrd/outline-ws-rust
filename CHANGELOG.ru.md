...
- Клиент VLESS-over-XHTTP packet-up (`vless_mode = "xhttp_h2"`). Парный листенер — в outline-ss-rust: на сессию генерируется случайный id, он же используется для обоих половин — driver открывает один long-lived GET (downlink) и пайплайнит POST'ы (uplink) с `X-Xhttp-Seq` поверх одного shared HTTP/2-соединения. Новое поле uplink-конфига `vless_xhttp_url` несёт базовый URL — обязательно, когда `vless_mode` — это один из `xhttp_*`. XHTTP-carrier выставлен через тот же `TransportStream` enum, что и WS-варианты, поэтому встраивается в текущий dial dispatch и в TUN / SOCKS-конвейеры без правок наверху. Полезен, когда WebSocket-апгрейд режется на сети (Cloudflare-style CDN, captive-portal middleboxes). `vless_mode = "xhttp_h3"` зарезервирован — пока возвращает явную "not yet implemented" ошибку вместо тихого fallback'а.
