...
- That workflow publishes a GitHub `prerelease` with built artifacts and `SHA256SUMS.txt`.
- To cut a stable release, run the manual `Release` workflow and pass `major_minor` such as `1.7`.
- CI finds the latest `v1.7.*` tag, increments the patch automatically, updates `Cargo.toml` and `Cargo.lock`, creates a release commit, creates tag `v1.7.Z`, and publishes a full GitHub Release with autogenerated release notes/changelog.
- Pushing a tag like `v1.2.3` manually still triggers the `Tag Release` workflow if you want an external tag-driven release path.
