...
- Этот workflow публикует GitHub `prerelease` с готовыми артефактами и `SHA256SUMS.txt`.
- Для стабильного релиза запускайте вручную workflow `Release` и передавайте `major_minor`, например `1.7`.
- CI находит последний тег вида `v1.7.*`, автоматически увеличивает patch, обновляет `Cargo.toml` и `Cargo.lock`, создает release-коммит, ставит тег `v1.7.Z` и публикует полноценный GitHub Release с autogenerated release notes/changelog.
- Если нужно, push тега вида `v1.2.3` по-прежнему запускает workflow `Tag Release` как внешний tag-driven путь публикации.
...
Для этого случая в репозитории добавлен отдельный manual workflow `MIPS Legacy Release`, который собирает `mips-unknown-linux-musl` и `mipsel-unknown-linux-musl` через nightly + `rust-src` + `-Z build-std`.
