...
- `[load_balancing] auto_failback` (по умолчанию `false`): управляет тем, возвращает ли прокси трафик на восстановившийся аплинк с более высоким приоритетом.
  ...
  - `true`: если текущий активный аплинк здоров, но существует кандидат с более высоким приоритетом, прокси может вернуть трафик на него — но только после того, как кандидат накопит `min_failures` последовательных успешных циклов проб. Этот порог стабильности исключает преждевременный возврат на primary, который ещё нестабильно восстанавливается (например, перезапускается).
...
2. Runtime- и probe-сбои добавляют затухающий штраф за сбой.
...
- `true`: если текущий активный здоров и существует кандидат с более высоким приоритетом, прокси может вернуть трафик на него — но только после накопления `min_failures` последовательных успешных циклов проб. Этот порог стабильности исключает преждевременный возврат на нестабильно восстанавливающийся primary (например, после перезапуска).
