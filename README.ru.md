...
- `prefixes` / `file`: inline-список и/или путь к файлу (один CIDR на строку, `#` комментарии и пустые строки игнорируются). Источники объединяются.
- `file_poll_secs`: как часто (в секундах) проверять mtime файла и перечитывать CIDR при изменении. По умолчанию `60`.
...
- `default = true`: ровно одно правило должно нести этот флаг; оно матчит всё, что не подошло по предыдущим правилам. `default`-правило не должно задавать `prefixes` или `file`.
...
- `/switch` — ручное переключение активного аплинка (POST)
...
группы не `active_passive`, и `405` для не-POST методов. Переопределение
держится, пока выбранный аплинк здоров; если проба позже отметит его
нездоровым, сработает обычный failover. При `auto_failback = true` цикл проб
может позже вернуться на более приоритетный аплинк, как только тот
стабилизируется.
