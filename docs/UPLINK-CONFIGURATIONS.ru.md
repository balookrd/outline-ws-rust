...
- Stream-one на h1 **не поддерживается** — `?mode=stream-one` с
  `vless_mode = xhttp_h1` (или цепочка, упавшая до h1) явно
  отвалится с понятной ошибкой на dial'е. Для stream-one используйте
  `xhttp_h2` / `xhttp_h3`.
...
- **stream-one** — один bidirectional POST: request body несёт
  ...
  половинки крутятся в отдельных tasks. **На `xhttp_h1` не
  поддерживается** — h1-carrier намеренно отваливается на
  `?mode=stream-one` вместо тихого даунгрейда до packet-up.

Оба submode'а идут через один и тот же `connect_xhttp` driver, так что
resume, fallback chain (`xhttp_h3 → xhttp_h2 → xhttp_h1` для
packet-up; `xhttp_h3 → xhttp_h2` для stream-one) и механика окна
...
1. **Per-host кэши** (короткий TTL, по одному на семейство).
   - `ws_mode_cache` — выставляется при падении h3/h2 WS handshake.
     ...
     `WsH2`).
   - `xhttp_mode_cache` — sibling-кэш для XHTTP-цепочки.
   ...
   Оба кэша ключатся по **назначению** `host:port` (dial-URL,
