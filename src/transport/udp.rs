...
use super::stream::{AnyWsStream, WsStream};

const MAX_UDP_SOCKET_PACKET_SIZE: usize = 65_507;
pub(super) const OVERSIZED_UDP_UPLINK_DROP_ERR: &str =
    "oversized UDP packet dropped before uplink send";

// ── Internal transport variant ────────────────────────────────────────────────
...
impl UdpWsTransport {
    ...
    pub async fn connect(
        ...
        let ws_stream = super::connect_websocket_with_source(url, mode, fwmark, ipv6_first, source)
            .await
