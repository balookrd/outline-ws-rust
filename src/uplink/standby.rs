...
impl UplinkManager {
    ...
    pub async fn try_take_tcp_standby(&self, candidate: &UplinkCandidate) -> Option<AnyWsStream> {
        ...
        crate::metrics::record_warm_standby_acquire("tcp", &candidate.uplink.name, "hit");
    ...
    pub async fn connect_tcp_ws_fresh(
        ...
        crate::metrics::record_warm_standby_acquire("tcp", &candidate.uplink.name, "miss");
    ...
    pub async fn acquire_udp_standby_or_connect(
        ...
        if candidate.uplink.transport == UplinkTransport::Shadowsocks {
            crate::metrics::record_warm_standby_acquire("udp", &candidate.uplink.name, "miss");
        ...
        if let Some(ws) = pool.udp.lock().await.pop_front() {
            ...
            crate::metrics::record_warm_standby_acquire("udp", &candidate.uplink.name, "hit");
    ...
    pub(super) async fn refill_pool(&self, index: usize, transport: TransportKind) {
        ...
        loop {
            ...
            match ws {
                Ok(ws) => {
                    ...
                    current_len += 1;
                    crate::metrics::record_warm_standby_refill(transport_label, &uplink.name, true);
                    debug!(
                        uplink = %uplink.name,
                        transport = ?transport,
                        desired,
    ...
    pub(super) async fn validate_pool(&self, index: usize, transport: TransportKind) {
        let desired = match transport {
            TransportKind::Tcp => self.inner.load_balancing.warm_standby_tcp,
            TransportKind::Udp => self.inner.load_balancing.warm_standby_udp,
        };
        ...
        while let Some(mut ws) = drained.pop_front() {
            ...
            let alive_result = super::probe_impl::validate_ws_stream_alive(&mut ws).await;
