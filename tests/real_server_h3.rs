...
impl ProxyProcess {
    fn start(config_path: &Path, log_path: &Path) -> Result<Self, Box<dyn std::error::Error>> {
...
struct UdpDomainPacket {
    host: String,
    port: u16,
    payload: Vec<u8>,
}

fn parse_udp_packet(packet: &[u8]) -> Result<UdpDomainPacket, Box<dyn std::error::Error>> {
...
fn build_dns_query(name: &str) -> Result<Vec<u8>, Box<dyn std::error::Error>> {
    let mut out = vec![
        0x12, 0x34, // id
...
fn reserve_tcp_port() -> Result<u16, Box<dyn std::error::Error>> {
